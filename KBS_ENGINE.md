...
| real-site action execution and result confirmation | KBS executor through Backend V1 breaker services |
...
run_kbs_cycles (Celery dispatcher)
  -> run_kbs_cycle_for_org
     -> services.run_cycle
        ...
        7. after commit, queue real-site actions through apps.kbs.executor
```
...
The database and KBS use only these canonical fields. Backend V1's former REST
keys (`priority`, `type`, `peak_load`, `mean_load`, and `protected`) remain as
serializer aliases so existing clients continue to work; writes through those
aliases are translated into the canonical fields. Migration
`breakers/0002_kbs_breaker_fields.py` preserves and renames existing data, and
`breakers/0006_merge_backend_kbs.py` joins the complete Backend V1 and KBS
migration histories.
...
| `POST /api/kbs/sim/ack/` | mark simulator actions executed; real-site actions are ignored |
...
- `apps/kbs/executor.py` is the real-device boundary. It claims committed
  intents, calls Backend V1's Tuya services, honors the active Tier-1 safety
  interlock, and persists applied/blocked/failed outcomes.
...
- production retry/monitoring hardening for device and broker outages.
