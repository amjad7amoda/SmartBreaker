"""Django/backend adapter for the dependency-free Tier-2 KBS."""
...
class DjangoKBSAdapter:
    """Translate between Django/backend services and the pure KBS contract."""
    ...
    def build_facts(self, organization, settings, cycle_time):
        ...
        window_minutes = max(
            settings.deficit_window_minutes,
        ...
        baseline_cutoff = cycle_time - timedelta(
            minutes=settings.baseline_minutes
        )
        baseline_rows = [
            row for row in readings
            if row.timestamp >= baseline_cutoff and row is not latest
        ]
        pv_baseline_W = mean([
            self._pv_power_W(row) for row in baseline_rows
        ])
        load_baseline_W = mean([
            row.ac_output_active_power_W for row in baseline_rows
        ])

        deficit_cutoff = cycle_time - timedelta(
            minutes=settings.deficit_window_minutes
        )
        deficit_J = joule_deficit_J([
            (
                row.timestamp,
                row.ac_output_active_power_W,
                self._pv_power_W(row),
            )
            for row in readings if row.timestamp >= deficit_cutoff
        ])

        active_event = organization.scheduled_events.filter(
            start_at__lte=cycle_time,
            end_at__gte=cycle_time,
        ).first()
        next_event = (
            organization.scheduled_events
            .filter(
                start_at__gt=cycle_time,
                start_at__lte=cycle_time + timedelta(
                    hours=settings.event_prep_hours
                ),
            )
            .order_by('start_at')
            .first()
        ...
        if active_event is not None:
            ...
            hours_until_event = (
                next_event.start_at - cycle_time
        ...
        if (
            ...
            and latest.battery_discharge_current_A is not None
        ):
            battery_draw_W = (
                latest.battery_voltage_V
                * latest.battery_discharge_current_A
            )
        else:
            battery_draw_W = max(load_now_W - pv_now_W, 0.0)
        battery_low = (
            latest.battery_voltage_V is not None
            and latest.battery_voltage_V
            <= settings.battery_low_voltage_V + settings.battery_low_margin_V
            and (latest.battery_charge_current_A or 0.0) <= 0.5
        )

        grid = next(
            (
                breaker for breaker in breaker_facts
                if breaker.priority_type == 'ac_grid'
            ),
            None,
        )
        grid_breaker_on = bool(grid and grid.switch)
        grid_energized = (
            (latest.grid_voltage_V or 0.0) >= settings.grid_present_min_V
        )
        sudden_draw = is_sudden_draw(
            load_now_W,
            load_baseline_W,
            settings.sudden_draw_W,
        )
        ...
        return SystemFacts(
            ...
            heat_high=(
                ...
                and latest.heatsink_temp_C
                >= settings.heatsink_temp_limit_C
            ),
            ...
            headroom_W=max(
                settings.max_inverter_power_W - load_now_W,
                0.0,
            ),
    ...
    def _pv_power_W(reading):
        ...
        if (
            reading.pv_input_voltage_V is not None
            and reading.pv_input_current_A is not None
        ):
    ...
    def _breaker_facts(breaker, cycle_time, event_required_ids):
        ...
        if (
            status is not None
            and status.switch
            and status.last_switched_on_at is not None
        ):
            minutes_since_on = (
                cycle_time - status.last_switched_on_at
            ).total_seconds() / 60.0
        recently_tripped = (
            not breaker.locked_out
            and breaker.locked_at is not None
            and cycle_time - breaker.locked_at
            < timedelta(hours=TRIP_MEMORY_HOURS)
        )
        return BreakerFacts(
            id=breaker.id,
            ...
            cur_power_W=(
                ...
                if status and status.cur_power_mW is not None
                else None
            ),
    ...
    @staticmethod
    def _sudden_draw_culprit_id(
        organization,
        cycle_time,
        baseline_minutes,
    ):
        window_start = cycle_time - timedelta(minutes=baseline_minutes)
        rows = (
