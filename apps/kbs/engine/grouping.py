...
Both functions take ``BreakerFact`` records: the motor inrush is already
folded into their ``expected_draw_W`` field, so this module only does set
math — no unit conversion, no timing.
"""
...
def first_group_within_headroom(candidates, headroom_W):
    """The subset of ``candidates`` safe to switch ON right now (list[BreakerFact]).
    ...
    candidates: breakers requested ON, most important considered first (BreakerFact)
    ...
    for breaker in sorted(candidates, key=importance_key):
        draw_W = breaker['expected_draw_W']  # power this breaker will pull right after switch-on (W)
...
def select_best_subset(candidates, budget_W):
    """The best subset of ``candidates`` to KEEP running within a power budget (list[BreakerFact]).
    ...
    candidates: currently-ON sheddable breakers (BreakerFact)
    ...
    for breaker in sorted(candidates, key=importance_key):
        draw_W = breaker['expected_draw_W']  # sustained draw of this breaker (W)
        if draw_W <= remaining_W:
            keep.append(breaker)
            remaining_W -= draw_W
    return keep
