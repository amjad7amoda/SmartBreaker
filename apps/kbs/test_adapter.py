...
class DjangoKBSAdapterTests(TestCase):
    ...
    @patch('apps.kbs.adapters.django._dispatch_real_actions')
    @patch(
        'apps.kbs.adapters.django.get_weather_context',
        return_value=WeatherContext(
            season='summer', condition='clear', sunrise=None, sunset=None
    ...
    def test_real_site_actions_dispatch_only_after_commit(
        self, _weather, dispatch,
    ):
        self.settings.data_source = 'real'
        self.settings.save(update_fields=['data_source'])
        facts = self.adapter.build_facts(
            self.organization, self.settings, self.cycle_time
        )
        result = RuleResult(
            branch='test.real-dispatch',
            actions=[ActionIntent(
                breaker_id=self.breaker.id,
                device_id=self.breaker.device_id,
                action='off',
                reason='real dispatch test',
            )],
        )

        with self.captureOnCommitCallbacks(execute=True):
            self.adapter.persist_result(
                self.organization, facts, result,
            )

        action_id = BreakerAction.objects.latest('id').id
        dispatch.assert_called_once_with((action_id,))
